...
import queue  # For command communication
import random

# import time # time.time() replaced with pygame.time.get_ticks() where appropriate
import pygame
...
class EyePair:
    ...
    def draw_smiling(self, screen):
        self.left_eye.draw_circular(screen, self.background_color, 10) # Slight upward curve for smile
...
class AnimationManager:
    ...
    def stop_concentrate(self):
        if self.current_state == AnimationState.CONCENTRATING: # Check if currently concentrating
            ...
            else: # If it was a timed concentrate, just end it early
                self.concentrate_start_time = self.current_time - (self.concentrate_duration + 1)
            # The _animate_concentrate logic will then pick up the expansion.
    ...
    def trigger_blinking(self):
        if self.current_state == AnimationState.IDLE:
            ...
            self.shrinking = True
            self.last_blink_time = self.current_time # Reset blink timer
            self.blink_interval = random.uniform(2000, 4000)
    ...
    def trigger_look(self):
        if self.current_state == AnimationState.IDLE:
            ...
            self.look_paused = False
            self.last_look_time = self.current_time # Reset look timer
            self.look_interval = random.uniform(10000, 20000)
    ...
    def _animate_blink(self):
        if self.shrinking:
            ...
            if self.eye_pair.left_eye.rect.height >= self.eye_pair.left_eye.original_rect.height:
                self.eye_pair.reset() # Resets size and position
                self.set_state(AnimationState.IDLE)
        ...
        if self.laugh_up:
            ...
            if self.laugh_offset <= 0:
                ...
                if self.laugh_cycle_count >= 4: 
                    self.set_state(AnimationState.IDLE) # This will trigger reset via set_state
    ...
    def _animate_sideways_look(self, direction):
        ...
        if self.look_paused:
            if self.current_time - self.look_pause_start_time > 1000: # Pause for 1 second
                self.look_paused = False
        ...
        if self.moving_away:
            ...
            if current_distance >= self.max_move_distance: # Use >= to ensure it triggers
            ...
            if abs(left_eye.rect.x - original_left_x) < self.move_speed : # Consider back if within one step
                self.eye_pair.reset()
                self.set_state(AnimationState.IDLE)
...
class MonkeyEyeApp:
    ...
    def _initialize_pygame_and_eyes(self):
        ...
        eye_y = self.screen_height // 2 - self.eye_height // 2
...
class EyesController:
    ...
    def stop_eyes(self):
        ...
        if self.eye_process:
            ...
            if self.eye_process.is_alive():
                ...
                self.eye_process.join(timeout=1) # Final wait
        self.eye_process = None
        self.command_queue = None
    ...
    def trigger_laugh(self): self._send_command("laugh")
    ...
    def trigger_concentrate(self, duration_ms=None, indefinite=False):
        ...
        self._send_command(cmd)
    def stop_concentrate(self): self._send_command("stop_concentrate")
